...
chrSel.forEach(element => {
	element.addEventListener("mouseover",
		() => {
			...
			positionCursor(chrSel.indexOf(element));
		}
	);
});
...
function setLocations() {
	...
	var iconPos = [1,0];
	autoResize("css-fullroster", selBrawler, chrCoords, portBrawler, iconPos, posBrawler);
	
	// Hitman
	chrCoords = [204,0, 327,0, 269,90, 145,90];
	iconPos = [145,0];
	autoResize("css-fullroster", selHitman, chrCoords, portHitman, iconPos, posHitman);
	
	// Swift Master
	chrCoords = [348,0, 471,0, 413,90, 289,90];
	iconPos = [289,0];
	autoResize("css-fullroster", selSwiftMaster, chrCoords, portSwiftMaster, iconPos, posSwiftMaster);
	
	// Ranger
	chrCoords = [492,0, 615,0, 557,90, 433,90];
	iconPos = [433,0];
	autoResize("css-fullroster", selRanger, chrCoords, portRanger, iconPos, posRanger);
	
	// Berserker
	chrCoords = [636,0, 759,0, 701,90, 577,90];
	iconPos = [577,0];
	autoResize("css-fullroster", selBerserker, chrCoords, portBerserker, iconPos, posBerserker);
	
	// Random
	chrCoords = [780,0, 852,0, 911,90, 721,90];
	iconPos = [721,0];
	autoResize("css-fullroster", selRandom, chrCoords, portRandom, iconPos, posRandom);
	
	// Vanguard
	chrCoords = [873,0, 996,0, 1055,90, 931,90];
	iconPos = [872,0];
	autoResize("css-fullroster", selVanguard, chrCoords, portVanguard, iconPos, posVanguard);
	
	// Launcher
	chrCoords = [1017,0, 1140,0, 1199,90, 1075,90];
	iconPos = [1016,0];
	autoResize("css-fullroster", selLauncher, chrCoords, portLauncher, iconPos, posLauncher);
	
	// Enchantress
	chrCoords = [1161,0, 1284,0, 1343,90, 1219,90];
	iconPos = [1160,0];
	autoResize("css-fullroster", selEnchantress, chrCoords, portEnchantress, iconPos, posEnchantress);
	
	// Trouble Shooter
	chrCoords = [1305,0, 1428,0, 1487,90, 1363,90];
	iconPos = [1304,0];
	autoResize("css-fullroster", selTroubleShooter, chrCoords, portTroubleShooter, iconPos, posTroubleShooter);
	
	// Battle Mage
	chrCoords = [1449,0, 1572,0, 1631,90, 1507,90];
	iconPos = [1448,0];
	autoResize("css-fullroster", selBattleMage, chrCoords, portBattleMage, iconPos, posBattleMage);
	
	// Monk
	chrCoords = [1,110, 124,110, 183,200, 59,200];
	iconPos = [0,110];
	autoResize("css-fullroster", selMonk, chrCoords, portMonk, iconPos, posMonk);
	
	// Dragon Knight
	chrCoords = [145,110, 268,110, 327,200, 203,200];
	iconPos = [144,110];
	autoResize("css-fullroster", selDragonKnight, chrCoords, portDragonKnight, iconPos, posDragonKnight);
	
	// Kunoichi
	chrCoords = [289,110, 412,110, 471,200, 347,200];
	iconPos = [288,110];
	autoResize("css-fullroster", selKunoichi, chrCoords, portKunoichi, iconPos, posKunoichi);
	
	// Inquisitor
	chrCoords = [433,110, 556,110, 615,200, 491,200];
	iconPos = [432,110];
	autoResize("css-fullroster", selInquisitor, chrCoords, portInquisitor, iconPos, posInquisitor);
	
	// Striker
	chrCoords = [577,110, 700,110, 759,200, 635,200];
	iconPos = [576,110];
	autoResize("css-fullroster", selStriker, chrCoords, portStriker, iconPos, posStriker);
	
	// Lost Warrior
	chrCoords = [720,110, 912,110, 853,200, 779,200];
	iconPos = [720,110];
	autoResize("css-fullroster", selLostWarrior, chrCoords, portLostWarrior, iconPos, posLostWarrior);
	
	// Grappler
	chrCoords = [932,110, 1055,110, 997,200, 873,200];
	iconPos = [873,110];
	autoResize("css-fullroster", selGrappler, chrCoords, portGrappler, iconPos, posGrappler);
	
	// Crusader
	chrCoords = [1076,110, 1199,110, 1141,200, 1017,200];
	iconPos = [1017,110];
	autoResize("css-fullroster", selCrusader, chrCoords, portCrusader, iconPos, posCrusader);
	
	// Ghostblade
	chrCoords = [1220,110, 1343,110, 1285,200, 1161,200];
	iconPos = [1161,110];
	autoResize("css-fullroster", selGhostblade, chrCoords, portGhostblade, iconPos, posGhostblade);
	
	// Spectre
	chrCoords = [1364,110, 1487,110, 1429,200, 1305,200];
	iconPos = [1305,110];
	autoResize("css-fullroster", selSpectre, chrCoords, portSpectre, iconPos, posSpectre);
	
	// Nen Master
	chrCoords = [1508,110, 1631,110, 1573,200, 1449,200];
	iconPos = [1449,110];
	autoResize("css-fullroster", selNenMaster, chrCoords, portNenMaster, iconPos, posNenMaster);
}
